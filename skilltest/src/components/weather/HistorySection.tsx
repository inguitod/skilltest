...
export function HistorySection({ entries, onSelectCity, onClear }: HistorySectionProps) {
	if (entries.length === 0) {
		return null;
	}
	...
	return (
		<Card className="border-border/60 bg-muted/25">
			<CardHeader className="flex flex-row items-center justify-between gap-4 space-y-0 pb-4">
				...
				{onClear ? (
					<CardAction>
						<Button variant="link" className="h-auto px-0 text-primary" onClick={onClear}>
							Clear
						</Button>
					</CardAction>
				) : null}
			...
			<CardContent className="grid grid-cols-1 gap-4 pt-0 md:grid-cols-2 lg:grid-cols-4">
				{entries.map((entry) => (
					<HistoryCard key={entry.id} entry={entry} onSelect={onSelectCity} />
				))}
			</CardContent>
		</Card>
	);
